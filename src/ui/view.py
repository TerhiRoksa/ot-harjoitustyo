...
class View:
    def __init__(self, master, calorie_counter):
        ...
        self.app_name_label = tk.Label(master, text="Painonhallintasovellus", font=app_name_font)
        ...
        self.button = tk.Button(master, text="Lisää ruoka", command=self.button_click)
        ...
        self.add_button = tk.Button(master, text="Lisää kalorit", command=self.add_calories)
        self.add_button.pack(pady=10)
  
        self.total_label = tk.Label(master, text=f"Kokonaiskalorit: {self.calorie_counter.get_total_calories():.2f}")
    ...
    def add_calories(self):
        try:
            ...
            self.total_label.config(text=f"Kokonaiskalorit: {self.calorie_counter.get_total_calories():.2f}")
        except ValueError:
            self.total_label.config(text="Virheellinen syöte. Anna kelvollinen luku.")
